  ...
  <div id="taskResult">
    ...
    <div class="main-content" v-if="data">
      ...
      <div class="box-item">
        ...
        <upload-box ref="upload" :data="imgData" :autoShow="false" @upload="uploadData" @deleteItem="deleteItem"
          :showDelete="deleteState" @clickimg="openImg" :initt="initt"></upload-box>
      </div>
      <p class="gang"></p>
      <div class="box-item">
        <span class="item-title">详细描述</span>
        <textarea rows="5" placeholder="请输入详细情况" v-model="contentText" v-if="!id"></textarea>
        <div class="remark-box" v-if="id" v-html="contentText"></div>
      </div>
      <!-- <div class="box-item">
        <span class="item-title">图片</span>
        <img class="pic-box" src="@/assets/images/pic.png" @click=""  />
        <upload-box ref="upload" :data="imgData" :autoShow="false" @upload="uploadData" @deleteItem="deleteItem"
          :showDelete="deleteState" @clickimg="openImg" :initt="initt"></upload-box>
      </div> -->
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      var taskState = store.get('taskState')
      // this.getTextlist()
      if (taskState == "detail") {
        this.moduleName = "任务执行结果"
        this.getData()
        this.getUser()
      } else if (taskState == "add") {
      ...
      .box-item {
        ...
        .item-ul {
          ...
          li {
            ...
            i.jump {
              background: #f8f8f8 url("../../../assets/images/right.png") no-repeat right center;
              background-size: auto 0.32rem;
              right: 0.4rem;
              position: absolute;
              display: block;
