      ...
      <div class="main-content-box">
        <div v-if="datalist">
          <div :class="['list', item.class]" v-for="(item, index) in datalist" :key="index">
            ...
            <input type="text" v-model="item.value" class="info" v-if="!item.type" />
            <my-select
            ...
            ></my-select>
            <my-area v-if="item.type === 'area'" :value="item.value" @submit="getArea"></my-area>
            <my-datetime-picker
              v-if="item.type === 'time'"
              v-model="item.value"
              type="date"
        ...
        <div class="listBtnGroup">
          ...
          <div class="delBtn listBtn" @click="deleteItem('productItems', 'deleteProductid')">
            <span></span>
            <span>删除</span>
          </div>
        </div>
        ...
        <div class="listBtnGroup">
          ...
          <div class="delBtn listBtn" @click="deleteItem('materialItmes', 'deleteRawMaterialByid')">
            <span></span>
            <span>删除</span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    submitBtn() {
      const text = this.$submitBefore(this.datalist[0],"value");
      if (text) {
        Toast(text);
        return false;
      }
    ...
    submitLngLat(e) {
      ...
      this.datalist.forEach(item => {
        if (item.key === "lnglat") {
          (item.value = [e.lng, e.lat]), (this.lnglatArr = item.value);
        }
      });
    }
  }
